...
if __name__ == '__main__':
    def main():
        ...
        with Pool() as pool:
            pool.starmap(download_image, [("downloaded_image_multiprocessing",int(files_count//process_count)) for _ in range(process_count)])
        
        end_t = time.perf_counter()
        
        total_duration = end_t - start_t
        print(f"Parellel-MultiProcessing Run: Downloaded files = {files_count} Total time taken {total_duration:.2f}s total")
